...
readme = '''Вводите ответы через запятую. Если этот критерий неважен, нажмите enter'''
print(readme)


user_platforms = input('Игры какой платформы вам интересны?\n').split(',')


try:
    user_age = int(input('Сколько вам лет?\n'))
except ValueError:
    user_age = int(input('Введите целое значение'))


user_categories = input('Какаие категории игр вам интересны?\n').split(',')


user_genres = input('Какой жанр вы ищете?\n').split(',')


user_rating = input('Важен ли рейтинг этой игры?\n если да, то поставьте +  ')
if user_rating == '+':
    user_rating = 1
else:
    user_rating = 0

user_price = [input('Минимальная стоимость игры?'), input('Максимальная стоимость игры')]
